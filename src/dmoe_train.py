...
class ScriptArguments:
    ...
    seed: int = field(default=42, metadata={"help": "Seed of training."})
    ...
    train_layer_idx: int = field(default=-1, metadata={"help": "The index of training layer to fine-tune."})
    ...
    map_to_expert: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to map each dataset to the corresponding expert."},
    )
    only_tune_router: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to tune the router weight only."},
    )
    router_aux_loss_coef: Optional[float] = field(
        default=-1,
        metadata={"help": "The training coefficient for load balance of router."},
    )
    eval_data_ratio: Optional[float] = field(
        default=0.02,
        metadata={"help": "The percentage of data used in the evaluation."},
    )
    consecutive_num: Optional[int] = field(
        default=48,
        metadata={"help": "The number of consecutive samples input to the expert model."},
    )
    dataset_key2id: Optional[str] = field(
        default="",
        metadata={"help": "The path to the mapping dict of dataset."},
    )
    only_tune_moe: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to tune the moe weight only."},
    )
...
def main(args):
    ...
    # Froze layer
    if args.train_layer_idx >= 0:
        train_layer_arr = decode_booleans(args.train_layer_idx, model.config.num_hidden_layers)
        for layer_i in range(model.config.num_hidden_layers):
            if not train_layer_arr[layer_i]:
                for p_n, p_v in model.named_parameters():
                    # prefixs = [".layers.x."#gemma, ".h.xx."#bloom]
                    if f".layers.{layer_i}." in p_n or f".h.{layer_i}." in p_n:
                        p_v.requires_grad = False

    # Finetune router only
    if args.only_tune_router:
        for p_n, p_v in model.named_parameters():
            if "gate.weight" not in p_n:
                p_v.requires_grad = False
                continue
            print(f"The gradient of parameters from {p_n} are tuned.")

    # Finetune moe related parameters only
    if args.only_tune_moe:
        for p_n, p_v in model.named_parameters():
            if "gate.weight" in p_n or "experts" in p_n:
                print(f"The parameters from {p_n} are tuned.")
                continue
            p_v.requires_grad = False
    ...
    # trainer
    trainer = Trainer(model=model, tokenizer=tokenizer, args=training_arguments, train_dataset=train_dataset, eval_dataset=eval_dataset)
